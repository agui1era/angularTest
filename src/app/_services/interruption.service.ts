...
export class InterruptionService {
  ...
  async refreshDataTable(id?,pag? ) {
    console.log(id);
    if (id) {
      this.http.get<any[]>(`${environment.apiUrl}/${this.entityPathName}/machine/${id}/0`).subscribe((ok: any) => {

        this.dataTableSubject.next(ok)
      })
    } else {
      this.http.get<any[]>(`${environment.apiUrl}/${this.entityPathName}?pag=${pag || 0}`).subscribe((ok: any) => {
        this.dataTableSubject.next(ok)
      })
    }
  ...
  fusionarTodo() {
    return this.http.get<any[]>(`${environment.apiUrl}/${this.entityPathName}/fusionar/todo`);
  }

  fusion(data){
    return this.http.post<any>(`${environment.apiUrl}/${this.entityPathName}/fusion`, data, {
      headers: {
  ...
  getByMachine(id,idturno) {
    return this.http.get<any>(`${environment.apiUrl}/${this.entityPathName}/machine/${id}/${idturno}`);
  }
  ...
  confirm(data) {
    return this.http.post<any>(`${environment.apiUrl}/${this.entityPathName}/confirmar`, data, {
      headers: {
        "content-type": "application/json"
      }
    });
