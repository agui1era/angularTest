...
export class SubproductmachineService {
...
*/
  dataManagerChangeValue(value) {
    this.dataTableSubject.next(value)
  }

  getAll(idmaquina) {
    return this.http.get<any[]>(`${environment.apiUrl}/subproduct/machine/${idmaquina}`);
  }
  create(data) {
    return this.http.post<any>(`${environment.apiUrl}/subproduct/machine`, data, {
      headers: {
  ...
  delete(id) {
    return this.http.delete<any>(`${environment.apiUrl}/subproduct/machine/${id}`, {
      headers: {
        "content-type": "application/json"
      }
    });
